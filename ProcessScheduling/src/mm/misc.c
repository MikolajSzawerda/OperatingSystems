...
PUBLIC int do_setpsched(){
  message m;
  int r;
  m.m1_i1 = who;
  m.m1_i2 = mm_in.m1_i1;
  m.m1_i3 = mm_in.m1_i2;
  r = _taskcall(SYSTASK, SYS_SETPSCHED, &m);
  if(r==0){
    return(OK);
  }
  return(EPERM);
...
PUBLIC int do_getppri(){
  message m;
  int r;
  m.m1_i1 = who;
  r = _taskcall(SYSTASK, SYS_GETPPRI, &m);
  if(r==0)
    return m.m1_i1;
  return(EINVAL);
}

PUBLIC int do_getpgid(){
  message m;
  int r;
  m.m1_i1 = who;
  r = _taskcall(SYSTASK, SYS_GETPGID, &m);
  if(r==0)
    return m.m1_i1;
  return(EINVAL);
}
