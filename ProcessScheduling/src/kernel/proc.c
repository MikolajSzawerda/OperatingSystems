 ...
 */
  ...
  if((rp = rdy_head[USER_RR]) != NIL_PROC){
    proc_ptr = bill_ptr = rp;
    return;
  }
  if((rp = best_p = rdy_head[USER_O]) != NIL_PROC){
    while(rp != NIL_PROC){
      if((rp->p_pri) > (best_p->p_pri)) best_p = rp;
      rp = rp->p_nextready;
    }
    proc_ptr = bill_ptr = best_p;
    best_p->p_pri = (best_p->p_pri > 1) ? best_p->p_pri-1 : 0;  
    return;
  }
  if((rp = best_p = rdy_head[USER_SJF]) != NIL_PROC){
    while(rp != NIL_PROC){
      if((rp->p_pri) < (best_p->p_pri)) best_p = rp;
      rp = rp->p_nextready;
    }
 ...
 */
  ...
  if (rdy_head[group] == NIL_PROC){
      rdy_head[group] = rdy_tail[group] = rp;
      rdy_tail[group]->p_nextready = NIL_PROC;
      return;
  }
  rdy_tail[group]->p_nextready = rp;
  rdy_tail[group] = rp;
  rdy_tail[group]->p_nextready = NIL_PROC;
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
    ...
    if ( (xp = rdy_head[group]) == NIL_PROC) return;
    if ( xp == rp){
        rdy_head[group] = xp->p_nextready;
        if(group==1){
          pick_proc();
          return;
        } 
...
PRIVATE void sched()
{
  register struct proc* p=NIL_PROC;
  if (rdy_head[USER_RR] != NIL_PROC){
    rdy_tail[USER_RR]->p_nextready = rdy_head[USER_RR];
    rdy_tail[USER_RR] = rdy_head[USER_RR];
    rdy_head[USER_RR] = rdy_head[USER_RR]->p_nextready;
    rdy_tail[USER_RR]->p_nextready = NIL_PROC;
  } else if((p=rdy_head[USER_O]) != NIL_PROC){
    while(p!=NIL_PROC){
      p->p_pri = p->p_pri + 1;
      p = p->p_nextready;
    }
