...
FORWARD _PROTOTYPE( int do_setpsched, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getppri, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getpgid, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETPSCHED:	r = do_setpsched(&m);	break;
	    case SYS_GETPPRI:	r = do_getppri(&m);	break;
	    case SYS_GETPGID:	r = do_getpgid(&m);	break;
	    default:		r = E_BAD_FCN;
	}
...
PRIVATE int do_setpsched(m_ptr)
register message *m_ptr;
{
  register struct proc* p;
  p = proc_addr(m_ptr->m1_i1);
  if(p->p_gid==1 && m_ptr->m1_i2 >=0 && m_ptr->m1_i3 >= 0){
    p->p_gid = m_ptr->m1_i2;
    p->p_pri = m_ptr->m1_i3;
    return(OK);
  }
