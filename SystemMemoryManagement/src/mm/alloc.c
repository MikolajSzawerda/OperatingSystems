...
PRIVATE int allocation_strategy = 0;
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *current_max, *prev_ptr_max;
  phys_clicks old_base;

  if(allocation_strategy){
    do {
      prev_ptr = prev_ptr_max = NIL_HOLE;
      hp = current_max = hole_head;
      while(hp != NIL_HOLE && hp->h_base < swap_base) {
        if(hp->h_len > current_max->h_len){
          current_max = hp;
          prev_ptr_max = prev_ptr;
        }
        prev_ptr = hp;
        hp = hp->h_next;
      }
      if(current_max->h_len >= clicks){
        old_base = current_max->h_base;
        current_max->h_base += clicks;
        current_max->h_len -= clicks;
        if (current_max->h_len == 0) del_slot(prev_ptr_max, current_max);
        return(old_base);
      }
    } while (swap_out());
  } else {
...
PUBLIC phys_clicks do_holemap(){
  phys_clicks buffer[NR_HOLES*2+1];
  register struct hole* hp;
  phys_clicks i=0;
  phys_clicks boundry = MIN((bf_size-1)/(sizeof(phys_clicks)*2), NR_HOLES*2+1);
  hp = hole_head;
  while(i<boundry && hp != NIL_HOLE && hp->h_base < swap_base){
    buffer[i++] = hp->h_len;
    buffer[i++] = hp->h_base;
    hp = hp->h_next;
  }
  ...
  sys_copy(MM_PROC_NR, D, (phys_bytes) buffer, who, D,
              (phys_bytes) addr, (phys_bytes) bf_size);
  return(i/2);
}

PUBLIC int do_worstfit(){
  allocation_strategy = fit_flag;
  return(0);
}
